...
import { getPosts, updateProfile } from '../../utils/api';

export default function Sidebar() {
  const { user, setUser } = useAuth(); // make sure useAuth exposes setUser or similar
  ...
  const coverUrl = coverPreview
    ? coverPreview
    : user.cover_path
      ? `${APP_URL}/storage/${user.cover_path}`
      : `${APP_URL}/images/default-cover.jpg`;
  ...
  const handleCoverChange = async (e) => {
    ...
    try {
      const res = await updateProfile({ cover: file }); // key = "cover"
  ...
  return (
    <div className="sidebar">
      ...
      {(hasBio || hasStatus || hasFrom || hasLivesIn || hasStudied || hasWork) && (
        <BgCard tag="intro">
          {hasBio && (
            <>
              <p>
